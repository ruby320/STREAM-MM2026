#!/usr/bin/env python3
"""
LLaMA-7B micro-batch sweep（DeepSpeed ZeRO-2, bf16, 默认随机初始化）。

对每个 (optimizer, micro_batch) 探测 OOM，记录 Peak / OptState / 吞吐。
用法见 run_microbatch_sweep.sh / run_fixed_global_batch_sweep.sh。
"""
...
from llama7b_common import (  # noqa: E402
    ...
    build_optimizer,
    create_accelerator,
    create_setup_accelerator,
    load_llama_model,
    reset_accelerator_singleton_state,
    seed_everything,
    sweep_meta_fields,
    sync_cuda,
    truncate_batch,
...
DEFAULT_SAVE_DIR = os.path.join(_7B_DIR, "results", "microbatch_sweep")
DEFAULT_FIXED_G_DIR = os.path.join(_7B_DIR, "results", "fixed_global_batch")
...
def parse_args():
    p = argparse.ArgumentParser(description="LLaMA-7B ZeRO-2 micro-batch / OptState sweep")
    p.add_argument(
        "--optimizer",
...
def _run_isolated_probe(args, micro: int, phase: str) -> dict:
    ...
    result = subprocess.run(cmd, cwd=_7B_DIR)
...
def _teardown_deepspeed_accelerator(accelerator) -> None:
    """销毁 DeepSpeed Accelerator 并重置 Accelerate 单例（保留 NCCL process group）。"""
    try:
        accelerator.free_memory()
    except Exception:
        pass
...
def _try_micro_config(
    ...
    try:
        ...
        layer_count = getattr(config, "num_hidden_layers", None) or getattr(
            config, "n_layer", 32
        )
...
def _run_benchmark_fixed_g_with_fallback(
    ...
    for row in try_order:
        ...
        accelerator = create_accelerator(grad_accumulation_steps=accum)
        try:
            if accelerator.is_main_process:
                accelerator.print(
            ...
            _teardown_deepspeed_accelerator(accelerator)
...
def _run_fixed_global_batch_sweep(args) -> None:
    ...
    init_acc = create_setup_accelerator()
...
def main():
    ...
    accelerator = create_accelerator(grad_accumulation_steps=args.grad_accum)
    seed_everything(args.seed)
    set_seed(args.seed)
